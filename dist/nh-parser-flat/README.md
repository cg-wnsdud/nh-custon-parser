...
- `requirements.txt` / `*.whl`: `requests`와 하위 의존성의 Python 3.11 오프라인 설치본

먼저 같은 폴더에서 `python -m pip install --no-index --find-links . -r requirements.txt`를 실행합니다.
...
HTTP 호출은 `requests`를 사용하며 필요한 wheel은 모두 이 폴더에 포함되어 있습니다.

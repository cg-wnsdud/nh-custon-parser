...
## 2.11 분석 요청 이후 값의 자동 연결과 현장 제공값

현재 `EtlClient.analyze()`는 다음 값을 앞 단계 응답에서 받아 다음 API에 그대로 연결한다.

| 단계 | 호출 | 다음 단계에 사용하는 응답값 |
|---|---|---|
| 1.16 | `POST /api/v1/etl/auto/start` | `data.file_paths[0]` |
| 1.17 | `GET /api/v1/file/info` | 쿼리 `file_path=data.file_paths[0]` |
| 1.18 | `GET /api/v1/file/result/list` | 쿼리 `docPath=data.file_paths[0]` |
| 1.19 | `GET /api/v1/file/result/doc` | 1.18에서 선택한 Default JSON의 `file_path`를 `docResultPath`로 사용 |

따라서 1.16이 계약에 맞는 `task_ids`, `file_paths`를 반환하면 1.17~1.19의 식별자는 사용자가
별도로 입력하지 않는다. `task_ids[0]`은 응답 계약 확인과 추적용으로 보존하지만, 가이드의
1.17~1.19 쿼리는 task ID가 아니라 file path를 사용한다. 상태 `002`가 될 때까지 자동
폴링하고, `999`, 알 수 없는 상태, timeout은 `parse()`의 `ERROR` 상태로 끝난다.

코드만으로 정할 수 없어 농협이 제공하거나 확인해야 하는 값은 다음과 같다.

- ETL 서버 base URL(IP/Port 포함)
- 1.16 필수값 `author`, `ws_id`
- 위 세 값을 KL `option`으로 주는지 플랫폼 환경변수로 주는지
- 농협 workspace에서 허용할 `prj_config` 값과 운영 timeout/폴링 정책
- 실제 결과 목록에서 Default JSON을 식별하는 파일명 규칙
- 지원 파일 확장자, 파일 크기와 페이지 수 제한
- 별도 HTTP 인증 헤더가 존재하는지 여부

특히 가이드 1.16~1.20에는 API Key나 토큰을 어느 헤더에 넣으라는 정의가 없다. 따라서
`parser_info.prop.key`를 임의 헤더로 전송하지 않는다. 농협이 별도 인증 규격을 제공하면
헤더명, 값 형식, 만료·갱신 방식을 받은 뒤 추가해야 한다. `callback_url`과 `meta_info`는
선택값이며 현재 폴링 방식에는 callback URL이 필요하지 않고 `meta_info`는 빈 객체로 보낸다.

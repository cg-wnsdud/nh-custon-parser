...
`doc_data.json`도 초기 등록 검증본에서는 출력하지 않는다. 후속 광고 후처리에서 광고
상품 유형, 적용 템플릿 등 문서 단위로 VectorDB에 반영할 값이 확정되면 후보로 사용한다.
이때 입력 `doc_data` 전체를 단순 복사하지 않고 KL의 전체/부분 갱신 계약에 맞춰 파서가
계산한 변경값을 구성한다.
...
플랫폼 외곽 코드는 원본 예제 보존을 우선한다. `main.py`의 `/parsing`,
`/parsing/result/{uuid}`, `/health` 흐름과 subprocess의 `parse()` 진입은 예제 골격을
유지하고, ETL 호출·정규화·HRC 변환 같은 프로젝트 로직은 `parsing_service.py` 이하에만
둔다. 예제의 미구현 option 처리와 명백한 실행 오류만 문서 계약에 맞게 최소 보정한다.

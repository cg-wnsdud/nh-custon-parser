...
- 농협 등록 화면이 평면 `.py`/`.whl`을 배치하는 실제 서버 경로와 기존
  `run-application.sh`의 작업 디렉터리 연결 방식
...
- 플랫폼 외곽은 원본 예제 골격을 유지하고 프로젝트 로직은 `parsing_service.py`의
  `parse(work_dir, img_dir, file_path, option)` 이하에 둔다.
- 원본 예제의 미구현 Base64 option 처리와 명백한 문법·실행 오류만 최소 보정한다.
- 개발 소스는 원본 예제 계층 구조를 유지하고, 등록용 `.py`/`.whl`만
  `dist/kl-flat/`에 평면화한다.
- 광고 상품 유형·적용 템플릿은 향후 `doc_data.json` 후보지만 초기 등록 검증본에는
  출력하지 않는다.
...
- [x] 1.16 응답 `file_paths[0]`을 1.17/1.18에 자동 전달
- [x] 1.18 Default JSON `file_path`를 1.19에 자동 전달
- [x] 가이드에 정의되지 않은 인증 헤더를 임의로 추가하지 않음
...
- [x] `.py`/`.whl`만 포함하는 `dist/kl-flat/` 평면 등록 후보본과 재생성 도구
- [ ] 지원 확장자 목록
- [ ] 알려진 제한사항
...
1. **등록 구조**
   ...
   - 업로드된 평면 `.py`/`.whl`이 서버의 어느 경로에 배치되는지
   - `run-application.sh`, `setup-application.sh`, 요구사항 파일은 플랫폼이 보유·실행하는지
...
3. **결과·적재 계약**
   ...
   - 파서가 계산해야 하는 `doc_data.json` 변경 항목과 전체/부분 갱신 방식
   - 광고 상품 유형·적용 템플릿을 `doc_data.json`의 어떤 key로 반영할지
